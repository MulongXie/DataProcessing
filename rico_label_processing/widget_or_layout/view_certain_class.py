...
def draw_node(node, board, layer, target_class, shrink_ratio=4):

    if re.search(target_class,  node['class']) is not None:
        ...
        cv2.putText(board, node['class'], (
        int((node['bounds'][0] + node['bounds'][2]) / 2) - 50, int((node['bounds'][1] + node['bounds'][3]) / 2)),
    ...
    for child in node['children']:
         draw_node(child, board, layer+1, target_class)
    return
...
if '__main__':
    ...
    while index < end:
        ...
        if os.path.exists(img_path) and os.path.exists(json_path):
            print(img_path)
            # extract Ui components, relabel themW
